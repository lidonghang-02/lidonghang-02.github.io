...
tags:
  - Linux
  - Linux内核驱动
source: https://blog.csdn.net/qq_66623299/article/details/140084939
description: jiffies（系统时钟中断计数器）是一个全局变量，用于跟踪自系统启动以来经过的“滴答数”（tick counts）。每个“滴答”对应一个时钟中断的周期，这个周期的长度由内核配置中的 HZ 值决定。因此，jiffies 可以用来计算两个时间点之间的时间差，或者确定某个事件自上次发生以来已经过了多少时间。由于 jiffies 是一个无符号整数，因此它有一个最大值（这取决于 jiffies 的数据类型和大小）。
---
